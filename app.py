...
with aba_avm:
    ...
    if st.session_state.memorizar_calculo is not None:
        ...
        m1.metric("Precisão das Árvores de Decisão (R²)", dados_calc['r2_score'])
        m2.metric("Amostras Brutas Lidas", f"{dados_calc['brutas']} {dados_calc['tipologia_limpa']}s")
        m3.metric("Amostras Homologadas (Pós-IQR)", f"{dados_calc['saneadas']} {dados_calc['tipologia_limpa']}s")
